# Workstream Test Spec: pi-ledger claim-ledger kernel
...
`pi-chart` tests and fixtures are brownfield evidence only. Future implementation may reuse planning patterns, but this spec intentionally uses a fresh synthetic kernel fixture and does not bind itself to current `EventEnvelope` behavior or `pi-chart` source layout.
...
The future implementation slice SHOULD define exact commands after inspecting repo scripts. Candidate closeout commands are:
...
Docs/planning-only closeout may additionally use:
