...
- Status: planning-ready draft from `$plan`.
- Source plan: `.omx/plans/plan-architecture-rebase-clinical-truth-substrate.md`.
- Scope: durable architecture decision and context-hygiene work before implementation.
...
The repository has a strong clinical truth substrate but has accumulated UI prototypes, memos, `.omx` planning artifacts, and draft connector docs that can mislead future coding agents. A durable architecture rebase decision is needed before broad implementation.
...
1. Publish or prepare ADR 018 stating the architecture rebase direction.
