...
Status: Draft v2 after Architect ITERATE. Architect concerns folded in: exact malformed-frontmatter severity table, full V-NOTES-01 routing, duplicate no-claimed-body semantics, explicit rn_agent warning, deterministic nurse_practitioner ambiguity policy. Source requirements: `.omx/specs/deep-interview-a6-a7-notes-translation-hitl.md` and prior final view-only ADR at `/home/ark/.claude/plans/ralplan-a6-a7-notes-translation.md`.
...
| `rn_agent` | Excluded; pi-agent system role |
...
- If `rn_agent` appears as a note or note-bearing communication `author.role`, emit an explicit `V-NOTES-02` warning and do not classify it as nursing. Do not rely on `checkAuthorSentinel`; that function only checks placeholder `author.id` values at `src/validate.ts:2445-2456`.
...
6. **V-NOTES-02 rn_agent excluded** — `rn_agent` does not classify as nursing; expect explicit `V-NOTES-02` warning and `ok === true` absent hard errors.
...
- `rn_agent` is excluded from nursing scope.
...
- Made `rn_agent` an explicit `V-NOTES-02` warning, not an author-sentinel side effect.
...
- Paired note + communication with the same `rn_agent` role: one rn_agent exclusion warning.
