...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `patient_001` | Revised-for-review candidate | Respiratory deterioration teaching case deepened to a 32-hour CAP arc with oxygen titration, allergy-aware antibiotic orderset and MAR, contested RT exam reconciliation, family communication, day-to-night handoff, AM repeat labs and CXR, hospitalist day-2 plan with PO transition target; pi-agent harness scaffolding (latent_release, live_expected, hidden_truth) added | Multi-shift 2026-04-18 → 2026-04-19 (32 hours, single encounter `enc_001`, ED → medicine ward → day-2 hospitalist rounds with discharge target tomorrow) | Hand-authored chart seed plus monitor-derived and nurse-charted vitals; no Synthea seed/version/parameters recorded | Machine pass: vitals/flowsheet trend (60+ samples across 2 days), bedside nursing assessment, serial notes (10 narratives), orders/meds/interventions including MAR with class hold, labs/diagnostics (admit + AM repeat + 2 CXR), care plan/handoff with explicit loop closures and contested-claim resolution | Present on events/vitals/labs/notes with effective/recorded/resulted/verified times, deterministic vital sample keys, and canonical evidence-ref shapes including vitals_window | Strong revised chain: worsening trend → provider respiratory orders → oxygen titration → POC labs → focused proof reassessment `evt_20260418T0842_02` → 09:00 vital + MD trend resolves SpO2-maintenance plan → 09:30 reassessment + MD trend resolves respiratory-watch plan → q4h monitoring plan → contested RT exam at 10:00 reconciled at 10:15 → admit core labs → allergy-aware antibiotic admin with class hold → afternoon trend → family update → day-to-night SBAR → overnight stable → AM repeat labs/CXR → hospitalist day-2 plan with PO transition criteria | Present (10): goals_of_care, follow-up nursing, 08:45 SBAR, 09:30 focused reassessment, RT consult, pharmacy med-rec, family update, day-to-night SBAR, day-2 morning handoff, hospitalist progress note | Present after rebuild; proof fact `evt_20260418T0842_02` reused across review/evidence/open-loop/handoff/narrative on day 1 and into day-2 morning handoff and hospitalist progress note | Pending operator review | No Synthea metadata; no operator signoff; revised candidate only | First fixture in corpus with explicit pi-agent harness scaffolding (latent_release, live_expected agent task queue + scoring rubric, hidden_truth boundary fixture); broadens seed evidence into multi-shift continuation with allergy-aware MAR, contested-claim reconciliation, and PO transition criteria; still insufficient for ADR 019 readiness alone |
