...
type ChartView = "overview" | "handoff" | "vitals" | "mar" | "notes" | "labs" | "radiology" | "agent";
...
function storyboardViews(): Record<ChartView, string> {
  return {
...
function renderWorklistSection(section: WorklistSection, index: number): string {
  ...
  return `<section class="worklist-section"><div class="worklist-section-head"><span>${index}</span><b>${escapeHtml(section)}</b><em>${sectionCount(section)}</em></div>${items}</section>`;
}
...
function renderNav(): string {
  const items: { view?: ChartView; label: string; count: string; alert?: boolean; group?: string }[] = [
