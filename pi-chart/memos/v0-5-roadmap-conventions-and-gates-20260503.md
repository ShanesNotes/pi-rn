...
- `.scratch/pi-chart-v0-5/` — active Matt-compatible V0.5 PRD, issue slices, triage state, and handoffs.
- `.omx/specs/` — interview/source artifacts produced by OMX workflows.
- `.omx/plans/` — RALPLAN/spec-prep workspace for Codex iteration and review history.
...
- `pi-chart/docs/adr/` — accepted ADRs only, numbered in repo-local sequence.

Working rule: `.omx/plans/` may be the live Codex planning substrate, but durable active work needs a `.scratch/<feature>/` PRD/issue surface before implementation handoff.
...
- Accepted ADR: `pi-chart/docs/adr/018-architecture-rebase-clinical-truth-substrate.md §Decision`.
...
- L1: The first accepted V0.5 kernel ADR is `pi-chart/docs/adr/019-v0-5-claim-ledger-kernel.md`, if approved.
...
- L6: No source/schema/package edits until Phase 0 durable artifacts plus Phase 1 PRD/test-spec are mirrored into `.scratch/pi-chart-v0-5/`, decomposed into approved issue slices, and Architect+Critic approval evidence is linked.
...
4. Mirror Phase 1 PRD/test-spec and review evidence into `.scratch/pi-chart-v0-5/`.
5. Decompose into Matt `$to-issues` tracer bullets before implementation.
...
Commit the existing dirty tree in scoped commits, then continue V0.5 work on a fresh tree. Do not stash or mix unrelated monitor/simulator cleanup with V0.5 planning commits. Active work should be visible in `.scratch`, not hidden in runtime-only `.omx` state.
